...
export default async function handler(req, res) {
  ...
  if (method === 'POST') {
    ...
    if (type === 'register') {
      ...
      await supabase.from('usernames').insert({ user_id: createdUser.user.id, username });
  ...
  if (method === 'GET') {
    ...
    try {
      ...
      const { data: user } = await supabase.from('users').select('id, email, created_at').eq('id', decoded.userId).maybeSingle();
      const { data: usernameRow } = await supabase.from('usernames').select('username').eq('user_id', decoded.userId).maybeSingle();
